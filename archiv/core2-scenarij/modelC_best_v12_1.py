"""
Model C — Best v12.1  (v12 + PRAVI Adadelta namesto Adam)
=============================================================
v12 je propadel (Core2 OA=46.97%, R0=0.00%!) — train_ll je obtical pri 0.64
po 6 epohah, kar kaze na OPTIMIZACIJSKO nestabilnost, ne na sibkejso
arhitekturo. Verjeten vzrok: clanek uporablja weight init N(0,0.02) in BREZ
BatchNorm SKUPAJ z Adadelta optimizerjem — Adadelta je adaptiven in
kompenzira slabo pogojenost, ki jo majhen init + brez BN + brez
StandardScaler ustvarijo. V v12 sem obdrzal Adam namesto Adadelta, s cimer
sem pretrgal ta usklajen recept.

v12.1 SPREMEMBA (edina razlika od v12):
  - Adadelta(lr=0.1) namesto Adam(lr=1e-3), tocno kot v clanku
  - BREZ CosineAnnealingLR urnika — clanek eksplicitno pravi: "Adadelta
    adapts the learning rate over time, removing the need to manually
    tune for our application." Dodajanje urnika nad Adadelta bi bilo v
    nasprotju s tem samim namenom optimizerja.

Vse ostalo identicno v12: single-stream CNN, brez BN, softplus, dropout=0.5,
weight init N(0,0.02), batch=128, brez StandardScaler, oversampling namesto
class weights, ista gnezdena Core2 validacija (Core 2 = pravi, enkratni test).

Ce to popravi konvergenco, je v12.1 pravo "zvesto" izhodisce za diplomo;
ce ne, je vzrok propada globlje v arhitekturi/receptu, ne v optimizerju.
"""
...
METODOLOGIJA_OPOMBA = (
    "v12.1 = v12 + PRAVI Adadelta(lr=0.1) namesto Adam, BREZ LR urnika "
    "(clanek: Adadelta 'removes the need to manually tune'). v12 je propadel "
    "(Core2 OA=46.97%, R0=0%, train_ll obtical pri 0.64) — verjetno zaradi "
    "optimizacijske nestabilnosti: weight init N(0,0.02) + brez BN + brez "
    "StandardScaler je slabo pogojen problem, ki ga clanek kompenzira prav "
    "z Adadelta. Ostalo identicno v12 (single-stream, brez BN, softplus, "
    "dropout=0.5, oversampling, ista gnezdena Core2 validacija). "
    "Ref. clanek CNN (SD, cross-slide, tezja naloga): OA=79.45%+/-1.25%."
)
...
# ---------------------------------------------------------------------------
# Splosen prostorski split (KMeans) — zunanji IN notranji split (identicno v10-v12)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Preprocessing — BREZ StandardScaler, samo PCA (kot v11/v12)
# ---------------------------------------------------------------------------
...
# Adadelta(lr=0.1), BREZ LR urnika (clanek: "removes need to manually tune")
# ---------------------------------------------------------------------------
...
def main():
    ...
    print(f"\n  Konfiguracija (v12.1 — zvesta replika clanka + pravi Adadelta):")
    print(f"    Single-stream CNN, brez BN, softplus, dropout={args.dropout}")
    print(f"    Adadelta(lr={args.lr}), BREZ LR urnika")
    ...
    print(f"  Best-of-N na INNER-VAL (Core 2 ni vpleten). Loss neutezen (oversampling). Adadelta.")
    ...
    print(f"  Optimizer: Adadelta(lr={args.lr}), brez LR urnika")
    ...
    print(f"    v10 (nasa gnezdena, dual-stream, Adam):       OA=73.82%  ll=0.84206")
    print(f"    v12 (single-stream, Adam — PROPADEL):         OA=46.97%  ll=1.54678")
    print(f"    v12.1 (single-stream, Adadelta):              OA={test_oa*100:.2f}%  ll={test_ll:.5f}")

    # ------------------------------------------------------------------
    print(f"\n=== 13. Zapis v {RESULTS_FILE} ===")
    write_results_report(
        model_name="modelC_best_v12_1",
