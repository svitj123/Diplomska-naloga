"""
Model C — Best v11  (Oversampling + odstranjen StandardScaler + popravljena primerjava)
=========================================================================================
Osnova: v10 (gnezdena validacija, Core 2 nikoli "pogledan" pred koncem).

DVE SPREMEMBI (izolirano testirani hipotezi iz clanka):

1. ODSTRANJEN StandardScaler.
   Preverjeno na realnih podatkih: polje `data` v HDF5 je ze baseline-korigirano
   IN Amide I normalizirano (vsak anotiran piksel ima min=0.0000 in vrednost
   TOCNO 1.0000 pri ~1648 cm-1). Rubber-band korekcija na teh podatkih je no-op
   (max|diff|=0.00000 na testiranih vzorcih). Clanek po baseline+Amide koraku
   direktno uporabi PCA(16) — brez z-score standardizacije. v11 to posnema:
   PCA fit direktno na (ze normaliziranih) podatkih, brez StandardScaler.

2. OVERSAMPLING namesto class-weighted loss.
   Clanek: "we stack copies of underrepresented classes... we use 100,000
   pixels for each class to train the CNN." Mi smo doslej uporabljali le
   utezeno loss funkcijo (class weights), NE oversamplinga. To je verjetno
   glavni razlog za perzistenten cross-core propad manjsinskih razredov
   (R0/R2/R3) skozi v6-v10. V11 oversampla vsak razred v train setu do
   velikosti najvecjega razreda (proporcionalno naši manjsi kolicini podatkov,
   ne dobesedno 100,000 kot v clanku), in loss postane NEUTEZEN (nn.NLLLoss()).

POPRAVLJENA PRIMERJAVA S CLANKOM:
  Prejsnje verzije (v8, v9, v10) so se primerjale z OA=56.41% — to je SVM
  rezultat (brez prostorske informacije), NE CNN! Pravi CNN (prostorsko +
  spektralno, primerljiv nasemu pristopu) doseze OA=79.45% +/- 1.25% na SD
  podatkih (Tabela 8 v clanku). Njihova ocena je cross-slide (trening na
  BR1003+BR2085b, test na BR961+BR1001 — popolnoma druge TMA plosce/pacienti),
  kar je TEZJA naloga od nase leave-one-core-out (isti slajd, en zadrzan krogec).
  Od v11 naprej rezultati_report.txt uporablja to popravljeno primerjavo.

Metodologija (nested validacija) ostaja identicna v10:
  ...
  Faza A: iskanje hiperparametrov (Core 2 se ne dotakne) — best-of-N na inner-val
  Faza B: finalni model (5 krogcev, brez peekanja) — en sam dotik s Core 2

Arhitektura = identicno v5/v8/v9/v10 (DualStreamCNN 302k, SPEC_DIM=748, 12 ensemble, sigma=1.5).
"""
...
# Popravljena referenca (glej opombo zgoraj): CNN (spektralno+prostorsko), ne SVM.
REF_CNN_SD_OA   = 79.45
REF_CNN_SD_STD  = 1.25
REF_SVM_SD_OA   = 56.41  # zgodovinsko napacno uporabljena referenca (v8-v10)

METODOLOGIJA_OPOMBA = (
    "v11 spremembe: (1) odstranjen StandardScaler — podatki so ze baseline+Amide I "
    "normalizirani s strani ponudnika, preverjeno empiricno (rubber-band je no-op); "
    "PCA fit direktno na normaliziranih podatkih, kot v clanku. "
    "(2) Oversampling manjsinskih razredov do velikosti najvecjega razreda namesto "
    "class-weighted loss (clanek: 'stack copies of underrepresented classes'). "
    "POPRAVLJENA PRIMERJAVA: prejsnje verzije so se primerjale z OA=56.41% (SVM, "
    "brez prostorske informacije) namesto pravega CNN=79.45%+/-1.25% (SD, cross-slide, "
    "Tabela 8 clanka). Njihova naloga (cross-slide) je tezja od nase (leave-one-core-out)."
)
...
# ---------------------------------------------------------------------------
def oversample_arrays(coords, spec, y, seed=42, verbose=True, label="train"):
    """
    Oversampla (z ponavljanjem, replace=True) vsak razred do velikosti
    najvecjega prisotnega razreda. Deterministicno (fiksen seed, neodvisen
    od ensemble seed-a modela) — vsi modeli v isti fazi vidijo isto
    oversamplano mnozico, razlike med njimi pridejo iz inicializacije,
    vrstnega reda shuffle-a in augmentacije.
    """
    ...
    return coords[idx], spec[idx], y[idx]


# ---------------------------------------------------------------------------
# Neighbourhood MEAN features
# ---------------------------------------------------------------------------
def extract_spec_features(flat_proc, H, W, coords, scales=NEIGH_SCALES):
    D        = flat_proc.shape[1]
    spec_map = flat_proc.reshape(H, W, D)
    r, c     = coords[:, 0], coords[:, 1]
    parts    = [flat_proc[r * W + c].copy()]
    for scale in scales:
        t0 = time.time()
...
# ---------------------------------------------------------------------------
# Preprocessing — BREZ StandardScaler (podatki so ze normalizirani)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Arhitektura: Dual-Stream CNN (identicno v5/v8/v9/v10)
# ---------------------------------------------------------------------------
...
def write_results_report(model_name, innerval_oa, innerval_ll, test_oa, test_ll,
    ...
    lines = [
        ...
        f"[popravljeno iz napacne SVM=56.41% ref. v v8-v10]"
        f"  -> {output_path}\n"
    ]
    if extra_note:
...
# Loss: NEUTEZEN (nn.NLLLoss()) — balans dosezen z oversamplingom, ne utezmi.
# ---------------------------------------------------------------------------
def train_single(data_pca, train_spec, val_ds, y_train, y_val,
                 train_coords, device, max_epochs, batch_size, lr, seed, spec_noise):
    """
    ...
    train_coords/train_spec/y_train so ze OVERSAMPLANI pred klicem.
    """
...
def train_blind(data_pca, train_spec, train_coords, y_train, device,
    ...
    dotakne dokler ni model povsem izurjen. Neutezen loss (oversampling).
    """
    torch.manual_seed(seed); random.seed(seed)
    train_ds = DualStreamDataset(data_pca, train_spec, train_coords, y_train,
                                  augment=True, spec_noise=spec_noise)
...
def main():
    parser = argparse.ArgumentParser(
        description="Model C Best v11: oversampling + brez StandardScaler + popravljena primerjava"
    )
    parser.add_argument("--input",           default="image1-competition.hdf5")
    parser.add_argument("--output",          default="modelC_best_v11_core2.npy")
    ...
    print(f"\n  Konfiguracija (v11 — oversampling, brez StandardScaler):")
    ...
    data_pca_A, flat_proc_A, pca_A = build_preprocessed(
    ...
    inner_train_spec = extract_spec_features(flat_proc_A, H, W, inner_train_coords)
    print(f"  Inner-val ({len(inner_val_coords):,} pikslov):")
    inner_val_spec   = extract_spec_features(flat_proc_A, H, W, inner_val_coords)
    inner_val_ds = DualStreamDataset(data_pca_A, inner_val_spec, inner_val_coords,
                                      y_inner_val, augment=False)

    print(f"\n=== 6. Faza A — Oversampling inner-train ===")
    it_coords_os, it_spec_os, y_it_os = oversample_arrays(
        inner_train_coords, inner_train_spec, y_inner_train,
        seed=args.seed, label="Faza A / inner-train")

    print(f"\n=== 7. Faza A — Ensemble ({args.n_ensemble} modelov x do {args.max_epochs} epoh) ===")
    ...
    for i, seed in enumerate(seeds):
        ...
        model, best_epoch = train_single(
            data_pca=data_pca_A, train_spec=it_spec_os,
            val_ds=inner_val_ds, y_train=y_it_os, y_val=y_inner_val,
    ...
    print("\n=== 8. Faza A — Temperature scaling (na inner-val) ===")
    T_opt = find_temperature(innerval_logits_avg, y_inner_val)

    print("\n=== 9. Faza A — evaluacija na inner-val ===")
    ...
    # ==================================================================
    print(f"\n=== 10. Faza B — Preprocessing (PCA fit na vseh 5 outer-train krogcih) ===")
    data_pca_B, flat_proc_B, pca_B = build_preprocessed(
    ...
    outer_train_spec = extract_spec_features(flat_proc_B, H, W, outer_train_coords)
    print(f"  Core 2 / test ({len(test_coords):,} pikslov):")
    test_spec = extract_spec_features(flat_proc_B, H, W, test_coords)

    print(f"\n=== 12. Faza B — Oversampling outer-train ===")
    ot_coords_os, ot_spec_os, y_ot_os = oversample_arrays(
        outer_train_coords, outer_train_spec, y_outer_train,
        seed=args.seed, label="Faza B / outer-train")

    print(f"\n=== 13. Faza B — Ensemble ({args.n_ensemble} modelov x {final_epochs} epoh, brez peeka) ===")
    ...
    for i, seed in enumerate(seeds):
        ...
        model_b = train_blind(
            data_pca=data_pca_B, train_spec=ot_spec_os,
            train_coords=ot_coords_os, y_train=y_ot_os,
            device=device,
    ...
    # ------------------------------------------------------------------
    print("\n=== 14. KONCNA evaluacija na Core 2 (edini dotik) ===")
    ...
    print(f"  (zgodovinsko napacna ref. SVM=56.41% — glej opombo v METODOLOGIJA_OPOMBA)")
    ...
    print(f"  Preprocessing: BREZ StandardScaler | Balans: oversampling (ne class weights)")
    ...
    print(f"\n  Primerjava z clankom (POPRAVLJENA):")
    print(f"    CNN (SD, cross-slide, tezja naloga): OA={REF_CNN_SD_OA:.2f}% +/- {REF_CNN_SD_STD:.2f}")
    print(f"    v11 (cross-core, nested, oversampling): OA={test_oa*100:.2f}%")
    print(f"    [SVM=56.41% je bila napacna ref. v v8-v10 — ni vec primerljiva]")

    # ------------------------------------------------------------------
    print(f"\n=== 15. Zapis v {RESULTS_FILE} ===")
    write_results_report(
        model_name="modelC_best_v11",
