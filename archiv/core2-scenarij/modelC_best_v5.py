"""
Model C — Best v5  (FLAGSHIP / one-shot submission)
===================================================
Cilj: premagati v2 (Final=0.40937, 3. mesto) in se pomakniti proti 1. mestu (0.40031).

Ta model je grajen na DOKAZANIH komponentah. Vsaka odločitev ima empirično
ali teoretično utemeljitev iz najinega dosedanjega dela + članka + Vida.

────────────────────────────────────────────────────────────────────────────
KAJ DELUJE (obdržimo iz v2 — Final=0.409):
  • DualStreamCNN: prostorni 17×17×16 PCA patch CNN + spektralni MLP
  • StandardScaler + PCA(16)  (članek: 16 PC = 90.03% variance SD)
  • Neighbourhood MEAN spektra — Vidova ideja, ki je dvignila v1→v2 (0.459→0.409)
  • Reflect padding, TTA (8 D4 simetrij)
  • Weighted NLLLoss (OA na redkih razredih) + temperature scaling (log loss)
  • Gaussian smoothing σ=1.5 na sklenjenem cropu  ← vreden ~0.058 log loss!

KAJ NE DELUJE (ne ponavljamo):
  • std/max/min neighbourhood (v3): noisy, overfitta specifičen šum slajda
    → v3 VAL ll=0.255 > v2 0.232, T=1.555 (slabo kalibrirano), Final=0.467
  • avto-tuning σ na val (v4): val piksli so razpršeni, luknje napolnimo s
    priorom → glajenje IZGLEDA škodljivo (σ→0), a na sklenjenem cropu POMAGA.
    → v4 σ=0 dal Final=0.467. ZATO: σ=1.5 TRDO KODIRANA.

EDINA NOVA STVAR (utemeljena, nizko tveganje):
  • Neighbourhood mean tudi pri SCALE 7 (poleg 3, 5):
    - Članek poudarja VEČSKALNO prostorsko-spektralno fuzijo.
    - Mean je DOKAZANO varna operacija (v2 uspeh), za razliko od max/min (v3).
    - Daje poln 187-pasovni regionalni podpis → komplementaren PCA-16 CNN-ju,
      ki spektralni detajl izgubi. Ni redundanca.
  • Ensemble 12 modelov (v2: 7) — čista redukcija variance, monotono pomaga log loss.

Pričakovanje: VAL ll ≲ 0.232, Final ~0.40 (cilj: vsaj 2. mesto, morda 1.).
"""
...
N_PCA        = 16
SPEC_DIM_RAW = 187
NEIGH_SCALES = [3, 5, 7]                                # ← v5: dodan scale 7
SPEC_DIM     = SPEC_DIM_RAW * (1 + len(NEIGH_SCALES))   # 187 × 4 = 748
...
# ---------------------------------------------------------------------------
# Prostorski 2-way split (80/20) — greedy po tkivnih komponentah
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Neighbourhood MEAN features (samo mean — dokazano varno; brez max/min)
# ---------------------------------------------------------------------------
def extract_spec_features(flat_sc, H, W, coords, scales=NEIGH_SCALES):
    """
    Za vsak piksel: [raw(187), mean_3(187), mean_5(187), mean_7(187)] = 748 dim.
    Mean povpreči šum → generalizira (za razliko od max/min, ki overfittata).
    Peak spomin: ~1.8 GB (flat_sc + en mean_map naenkrat).
    """
    D        = flat_sc.shape[1]
...
class DualStreamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and self.spec_noise > 0:
            # Perturbira samo RAW del spektra (prvih 187); mean-feature pustimo pri miru
...
# ---------------------------------------------------------------------------
# Arhitektura: Dual-Stream CNN (identična v2, spektralni vhod = 748)
# ---------------------------------------------------------------------------
class DualStreamCNN(nn.Module):
    """
    Prostorni tok:  17×17×16 PCA patch → CNN → 64 feat  ("kako izgleda okolica")
    Spektralni tok: 748-dim [raw + mean@3,5,7] → MLP → 64 feat  ("kemija + regija")
...
def get_logits_tta(model, data_pca, spec_dense, coords, labels, device, batch_size=512):
    """TTA: povprečenje logitov čez 8 D4 simetrij → manjša varianca napovedi."""
    logits_sum = None
...
def find_temperature(logits, y):
    """Post-hoc kalibracija: poišče T, ki minimizira log loss na val."""
    def neg_ll(T):
        s = logits / T
...
def gaussian_smooth_probs(probs, tissue_mask, sigma):
    """Maskirano Gaussian glajenje verjetnosti — sosedje iz istega tkiva se uskladijo."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Model C Best v5 (flagship): dual-stream + multi-scale mean + 12 ensemble"
    )
    parser.add_argument("--input",      default="image1-competition.hdf5")
    parser.add_argument("--output",     default="modelC_best_v5.npy")
    ...
    parser.add_argument("--sigma",      type=float, default=1.5,
                        help="Gaussian smoothing sigma. PRIVZETO 1.5 (dokazano deluje). "
                             "NE auto-tunaj na val — to je metodološko napačno (glej v4).")
    parser.add_argument("--min-final-epochs", type=int, default=15,
                        help="Min epoh za finalni 100%% trening (v2 dokaz: 15).")
    ...
    print(f"\n  Konfiguracija (FLAGSHIP):")
    print(f"    SPEC_DIM={SPEC_DIM}: raw(187) + mean@{NEIGH_SCALES} (samo mean, brez max/min)")
    print(f"    Patch {PATCH_SIZE}×{PATCH_SIZE} reflect | PCA({N_PCA}) | TTA 8 | Ensemble {args.n_ensemble}")
    print(f"    σ={args.sigma} FIKSNA (proven) | weighted loss + temperature")
    print(f"    DualStreamCNN ({n_param:,} param)")

    # ------------------------------------------------------------------
    print("\n=== 2. Prostorski 80/20 split ===")
    train_mask, val_mask = make_spatial_two_way_split(
        tissue_mask, classes, prediction_crop_mask, args.val_frac)
    ...
    print(f"  Ref v2 (best): VAL ll=0.23217  Final=0.40937")

    sigma_opt = args.sigma   # FIKSNA — ne tunamo (val struktura ni reprezentativna)
    ...
    print(f"  Gaussian smoothing (σ={sigma_opt:.1f}, FIKSNA)...")
    if sigma_opt > 0:
    ...
    print(f"  T={T_opt:.4f} | σ={sigma_opt:.1f} (fiksna)")
    ...
    print(f"    modelC_best_v2: 0.40937  ← dosedanji best")
