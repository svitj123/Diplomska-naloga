"""
Model C — Best v8  (Cross-core validacija s fiksnimi epohi)
============================================================
Osnova: v6 (KMeans cross-core split), popravek: odstranjen early stopping.

NAPAKA v6:
  Enacili smo "val set za early stopping" z "val set za evalvacijo".
  Cross-core val je iz druge distribucije kot train -> model ne napreduje
  na val coru po epohi 1 -> early stopping sproži pri ep.1 -> podučen model.
  Ep.1 ni bug — model res doseže best cross-core napoved zgodaj, ampak to
  ne pomeni da je dovolj naučen.

RESITEV v8:
  Locimo dve stvari ki sva jih mesali:
    PREJ: val krogec -> early stopping + T kalibracija + metrike (vse skupaj)
    ZDAJ: fiksnih N epoh -> treniranje (brez early stopping)
          val krogec    -> SAMO T kalibracija + metrike

  Best-of-N selekcija: treniramo tocno eval_epochs epoh, shranimo stanje z
  najnizjim val loss med vsemi epohi (ne ustavljamo, le izberemo najboljše).

UTEMELJITEV N=6 epoh (za mentorja):
  Optimalno stevilo epoh doloceno z within-core validacijo (v5: konvergenca
  pri povprecno 4 epohi na 80% podatkov). Za cross-core evalvacijo smo
  fiksirali 6 epoh in val krogec uporabili izkljucno za kalibracijo
  temperature in porocanje metrik — ne za early stopping.

Arhitektura = identicno v5 (DualStreamCNN 302k, SPEC_DIM=748, 12 ensemble, sigma=1.5).
"""
...
METODOLOGIJA_OPOMBA = (
    "Optimalno stevilo epoh doloceno z within-core validacijo (v5: konvergenca "
    "pri povprecno 4 epohi na 80% podatkov). Val krogec uporabljen izkljucno za "
    "kalibracijo temperature in porocanje metrik — ne za early stopping."
)
...
# ---------------------------------------------------------------------------
# Leave-One-Core-Out split (KMeans) — identicno v6
# ---------------------------------------------------------------------------
def make_spatial_two_way_split(tissue_mask, classes, prediction_crop_mask,
    ...
    Val = najvecji core z vsemi 6 razredi -> SAMO za T + metrike.
...
# ---------------------------------------------------------------------------
# Arhitektura: Dual-Stream CNN (identicno v5)
# ---------------------------------------------------------------------------
...
def write_results_report(model_name, val_oa, val_ll, output_path, t_opt, sigma,
                          eval_epochs, best_epoch, extra_note=""):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = [
        f"{timestamp}  {model_name:<25}  "
        f"VAL_OA={val_oa*100:6.2f}%  VAL_ll={val_ll:.5f}  "
        f"T={t_opt:.4f}  sigma={sigma:.1f}  eval_ep={eval_epochs}(best={best_epoch})"
        f"  -> {output_path}\n"
    ]
    if extra_note:
...
def train_single(data_pca, train_spec, val_ds, y_train, y_val,
    ...
    """
    Trena tocno eval_epochs epoh. Brez early stopping.
...
def main():
    parser = argparse.ArgumentParser(
        description="Model C Best v8: cross-core split, fiksne epohe (brez early stopping)"
    )
    parser.add_argument("--input",           default="image1-competition.hdf5")
    parser.add_argument("--output",          default="modelC_best_v8.npy")
    parser.add_argument("--eval-epochs",     type=int,   default=6,
                        help="Stevilo epoh za eval fazo. Doloceno iz v5 (avg best=4) + buffer.")
    ...
    print(f"\n  Konfiguracija (v8 — cross-core, fiksnih {args.eval_epochs} epoh):")
    ...
    print(f"    eval_epochs={args.eval_epochs} FIKSNO (brez early stopping)")
    ...
    # ------------------------------------------------------------------
    print(f"\n=== 5. Ensemble trening ({args.n_ensemble} modelov x {args.eval_epochs} epoh) ===")
    print(f"  Brez early stopping — val krogec samo za T in metrike.")
    ...
    # ------------------------------------------------------------------
    print("\n=== 7. Val core evaluacija (cross-core) ===")
    ...
    print(f"  Ref v6: VAL OA=74.17%  VAL ll=0.77597  (ep=1, podučen)")
    print(f"  Ref v5: VAL OA=92.35%  VAL ll=0.23094  (within-core, za inf.)")
    ...
    print("\n=== POVZETEK (v8) ===")
    print(f"  eval_epochs={args.eval_epochs} (fiksno, brez early stopping)")
    print(f"  Best-of-{args.eval_epochs}: {best_epochs} (avg={avg_best_epoch})")
    print(f"  T={T_opt:.4f} | sigma={sigma_opt:.1f} | final_epochs={final_epochs}")
    print(f"  VAL OA={val_oa*100:.2f}%  VAL ll={val_ll:.5f}  (cross-core)")
    print(f"\n  Primerjava (cross-core val, nizje = boljse):")
    print(f"    v6 (ep=1, podučen):  VAL OA=74.17%  VAL ll=0.77597  Final=0.40396")
    print(f"    v8 (ep={args.eval_epochs}, fiksno):  VAL OA={val_oa*100:.2f}%  VAL ll={val_ll:.5f}  Final=???")
    print(f"\n  Submission: {args.output}")

    # ------------------------------------------------------------------
    print(f"\n=== 10. Zapis v {RESULTS_FILE} ===")
    write_results_report(
        model_name="modelC_best_v8",
