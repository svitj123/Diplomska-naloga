"""
Model D-hybrid — Cross-Slide Hibridni CNN-ViT (konvolucijski stem + transformer)
====================================================================================
OZADJE: prvi cisti ViT poskus (modelD_transformer_crossSlide.py) je, KO SMO MU
POVECALI KAPACITETO (192/8/6 -> 256/10/8, + vec train podatkov + mocnejsa
regularizacija), rezultat POSLABSAL (OA 52.82% -> 52.08%), s hudim kolapsom
R1/R4 (71.75%/76.51% -> 34.32%/36.64%). Diagnoza: problem ni bil premajhna
kapaciteta, ampak manjkajoc prostorski induktivni prior -- cisti ViT nima
translacijske ekvivariance/lokalnosti, ki jo CNN dobi "zastonj" iz konvolucije,
in se je s SE VEC parametri le se bolj overfittal na napacne stvari.

POPRAVEK (ta skripta): namesto povecevanja ViT kapacitete dodamo majhen
KONVOLUCIJSKI STEM (Conv32->Conv64, 3x3, padding=1, BREZ pooling-a) PRED
tokenizacijo. Ideja je uveljavljena v literaturi ("Early Convolutions Help
Transformers See Better", Xiao et al. 2021) -- nekaj zgodnjih konvolucijskih
plasti pred ViT stabilizira optimizacijo in vsili lokalnost, ki jo self-
attention sam po sebi nima, se posebej pri malo podatkov (kot tukaj). Self-
attention nato dela na ze prostorsko-obdelanih znacilkah (64-kanalni feature
mapi), namesto na surovih PCA vrednostih -- kombinira CNN-jevo lokalno
induktivno pristranskost s ViT-jevo sposobnostjo globalnega/adaptivnega
konteksta.

Kapaciteta te razlicice: NAMENOMA nazaj na manjso (192/8/6, kot prvi/originalni
ViT poskus) -- fokus je na popravku arhitekture (induktivna pristranskost), ne
na se vecji surovi kapaciteti (ki se je dvakrat izkazala za napacno smer).

ARHITEKTURA:
  1. Vhod: 33x33x16 PCA patch
  2. NOVO: Konvolucijski stem (Conv2d(16,32,3,pad=1)->GELU->Conv2d(32,64,3,pad=1)
     ->GELU), spatial dimenzija OSTANE 33x33 (brez pooling-a, da token-grid
     ostane nespremenjen)
  3. Patch embedding: ne-prekrivajoci 3x3 sub-patch-i NA STEM IZHODU (64 kan.),
     mreza 11x11=121 tokenov, linearna projekcija v embed_dim
  4. Ucljivi [CLS] token + ucljivi pozicijski embeddingi
  5. N standardnih transformer encoder plasti (multi-head self-attention +
     MLP, pre-LN, residual povezave)
  6. Klasifikacijska glava: LayerNorm + Linear na CLS token izhodu

TRENING: AdamW + cosine schedule z warmup, label smoothing za regularizacijo.
...
je podedovano iz modelC_crossSlide_faithful.py / modelD_transformer_crossSlide.py
-- ista metodologija, primerljivi rezultati, samo arhitektura je nova.
"""
...
METODOLOGIJA_OPOMBA = (
    "modelD_transformer_hybrid: HIBRIDNI CNN-ViT (konvolucijski stem Conv32->"
    "Conv64 BREZ pooling-a PRED tokenizacijo, nato 3x3 sub-patch tokeni=121, "
    "CLS token, N encoder plasti, multi-head self-attention) na PRAVEM "
    "cross-slide splitu. Popravek po neuspehu cistega ViT-a: povecanje "
    "kapacitete (192/8/6->256/10/8) je poslabsalo rezultat (OA 52.82%->"
    "52.08%, kolaps R1/R4) -- problem ni bila kapaciteta, ampak manjkajoc "
    "prostorski induktivni prior. Konvolucijski stem vsili lokalnost pred "
    "self-attention (Xiao et al. 2021, 'Early Convolutions Help Transformers "
    "See Better'). Kapaciteta namenoma nazaj na 192/8/6. AdamW+cosine+warmup, "
...
class ConvStem(nn.Module):
    """Nekaj zgodnjih konvolucijskih plasti PRED ViT tokenizacijo (Xiao et al.
    2021, 'Early Convolutions Help Transformers See Better') -- vsili lokalno
    prostorsko procesiranje, ki ga cisti ViT nima. BREZ pooling-a, da spatial
    dimenzija (in s tem token-grid) ostane nespremenjena."""
    def __init__(self, n_channels, stem_channels=64, dropout=0.1):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(n_channels, 32, 3, padding=1),
            nn.GELU(),
            nn.Dropout2d(dropout),
            nn.Conv2d(32, stem_channels, 3, padding=1),
            nn.GELU(),
            nn.Dropout2d(dropout),
        )
...
class SpatialSpectralViT(nn.Module):
    def __init__(self, patch_size=PATCH_SIZE, token_size=TOKEN_SIZE, n_channels=N_PCA,
                 ...
                 mlp_ratio=4.0, dropout=0.1, stem_channels=64):
        super().__init__()
        ...
        # NOVO: konvolucijski stem pred tokenizacijo (glej ConvStem docstring).
        self.stem = ConvStem(n_channels, stem_channels=stem_channels, dropout=dropout)

        # "Patchify": ne-prekrivajoci token_size x token_size sub-patch-i NA
        # STEM IZHODU (ne na surovem PCA vhodu vec), linearna projekcija
        # (implementirano kot Conv2d s stride=kernel=token_size, standarden
        # ViT patch-embedding trik).
        self.patch_embed = nn.Conv2d(stem_channels, embed_dim,
    ...
    def get_logits(self, patch):
        B = patch.size(0)
        stem_out = self.stem(patch)                     # (B, stem_channels, patch, patch)
        tokens = self.patch_embed(stem_out)             # (B, embed_dim, grid, grid)
...
def main():
    ...
    parser.add_argument("--output",     default="modelD_transformer_hybrid_test.npy")
    ...
    write_results_report(
        model_name="modelD_transformer_hybrid",
