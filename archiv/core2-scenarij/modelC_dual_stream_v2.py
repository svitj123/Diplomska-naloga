"""
Model C — Dual-Stream CNN v2 (ensemble + dual pooling + spektralni šum)
========================================================================
Izboljšave glede na v1:

  1. ENSEMBLE (najpomembnejše)
     Treniramo N modelov z različnimi seedi, povprečimo verjetnosti.
     Matematično: avg(p_1, p_2, ..., p_N) je vedno boljše kalibrirano
     kot katerikoli posamičen model. V praksi: -0.03 do -0.05 na log loss.
     Čas: ~7 min na model → 3 modeli = ~21 min.

  2. DUAL POOLING v prostornem toku
     Namesto samo AvgPool → [AvgPool || MaxPool] = 128 prostornih značilk.
     AvgPool: povprečna aktivacija → globalne tkivne lastnosti
     MaxPool: max aktivacija → najizrazitejša lokalna lastnost
     Skupaj dasta bogatejšo reprezentacijo.

  3. SPEKTRALNI ŠUM (augmentacija)
     Med treningom dodamo šum σ=0.01 na centralni spekter.
     Spectral MLP se ne more naučiti na pamet točnih vrednosti.
     Na valu/testu: brez šuma.

  4. N_PCA=32 (namesto 16)
     Prostorni tok vidi 32 namesto 16 PCA komponent → več spektralne
     informacije v patchu. Skupaj z direktnim spektrom: maximalna pokritost.

  5. patience=20 (namesto 15)
     Damo modelu več časa da konvergira.

Architektura Dual-Stream v2:
  Prostorni tok:
    Conv2d(32→64, k=3) + BN + ReLU + Drop + MaxPool
    Conv2d(64→128, k=3) + BN + ReLU + Drop
    Conv2d(128→128, k=3) + BN + ReLU + Drop + MaxPool
    AvgPool(1) → 128 | MaxPool(1) → 128 | Concat → 256
...
class DualStreamDataset(Dataset):
    """
    Za vsak piksel vrne:
      patch_t:    (N_PCA, 33, 33) — za prostorni tok
      spectrum_t: (187,)          — za spektralni tok
      label_t:    int

    Augmentacija (samo train):
      - Flip H/V, rot90 na patchu
      - Gaussov šum σ=0.01 na spektru (novo v v2)
    """

    def __init__(self, data_pca, flat_pp, coords, labels,
                 image_width, patch_size=PATCH_SIZE,
                 augment=False, spec_noise=0.01):
...
# ---------------------------------------------------------------------------
# Arhitektura: DualStreamCNN v2 (dual pooling, N_PCA=32)
# ---------------------------------------------------------------------------
class DualStreamCNN(nn.Module):
    """
    Dual-Stream v2:

    Prostorni tok (patch 33×33×32):
      Conv(32→64,  k=3) + BN + ReLU + Drop(0.3) + MaxPool(2) → (64, 16, 16)
      Conv(64→128, k=3) + BN + ReLU + Drop(0.3)              → (128, 16, 16)
      Conv(128→128,k=3) + BN + ReLU + Drop(0.3) + MaxPool(2) → (128, 8, 8)
      AvgPool(1) → 128 | MaxPool(1) → 128 | Concat → 256

    Spektralni tok (187 dim):
      Linear(187→256) + BN + ReLU + Drop(0.4)
      Linear(256→128) + BN + ReLU + Drop(0.3)
      Linear(128→64)  + BN + ReLU → 64
    ...
    def __init__(self, n_channels=N_PCA, spec_dim=SPEC_DIM,
        ...
        # Dual pooling: avg + max → 2×128 = 256 spatial features
    ...
    def get_spatial_features(self, patch):
        x = self.spatial_cnn(patch)           # (B, 128, 8, 8)
        avg_f = self.avg_pool(x).squeeze(-1).squeeze(-1)  # (B, 128)
        max_f = self.max_pool(x).squeeze(-1).squeeze(-1)  # (B, 128)
        return torch.cat([avg_f, max_f], dim=1)            # (B, 256)

    def get_spectral_features(self, spectrum):
        return self.spectral_mlp(spectrum)    # (B, 64)

    def get_logits(self, patch, spectrum):
        f_sp  = self.get_spatial_features(patch)
        f_spc = self.get_spectral_features(spectrum)
        return self.fusion(torch.cat([f_sp, f_spc], dim=1))
...
def get_logits_array(model, dataset, device, batch_size=512):
    ...
    all_l = []
    for patches, spectra, _ in loader:
        all_l.append(model.get_logits(patches.to(device),
                                       spectra.to(device)).cpu().numpy())
...
# Temperature scaling
# ---------------------------------------------------------------------------
def find_temperature(logits, y_val):
    def neg_ll(T):
        s = logits / T
...
# ---------------------------------------------------------------------------
def train_single_model(data_pca, flat_pp, W,
                       train_coords, y_train,
                       val_coords, y_val,
                       device, weights,
                       epochs=150, batch_size=128, lr=1e-4,
                       patience=20, seed=42, spec_noise=0.01):
    ...
    print(f"  {'Epoha':>6}  {'Train ll':>10}  {'Val OA':>9}  {'Val ll':>9}  {'LR':>9}")
    print(f"  {'─'*6}  {'─'*10}  {'─'*9}  {'─'*9}  {'─'*9}")
    ...
    print(f"  Naložene uteži: epoha={best_epoch}, val_ll={best_val_loss:.5f}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Dual-Stream CNN v2 (ensemble + dual pooling + spektralni šum)"
    )
    parser.add_argument("--input",              default="image1-competition.hdf5")
    parser.add_argument("--output",             default="modelC_dual_v2.npy")
    ...
    parser.add_argument("--n-ensemble",         type=int,   default=3,
                        help="Število modelov v ensemblu (priporočeno: 3-5)")
    ...
    print(f"\n  DualStreamCNN v2: {n_param:,} param")
    print(f"  Ensemble: {args.n_ensemble} modelov | seedi: "
          f"{list(range(args.seed, args.seed + args.n_ensemble))}")

    # ------------------------------------------------------------------
    print(f"\n=== 4. Ensemble trening ({args.n_ensemble} modelov) ===")

    # Zbiramo logite za val in test (za ensemble temperature scaling)
    val_logits_list  = []
    test_logits_list = []
    best_epochs      = []
    ...
    for i, seed in enumerate(range(args.seed, args.seed + args.n_ensemble)):
        print(f"\n  ── Model {i+1}/{args.n_ensemble} (seed={seed}) ──")
        model, best_epoch, _ = train_single_model(
    ...
    # Ensemble: povprečenje logitov pred temperature scalingom
    ...
    n_all_pixels   = int((classes != -1) & (~prediction_crop_mask)).sum() if hasattr(int, '__call__') else int(((classes != -1) & (~prediction_crop_mask)).sum())
    n_all_pixels   = int(((classes != -1) & (~prediction_crop_mask)).sum())
    final_epochs   = max(avg_best_epoch, round(avg_best_epoch * n_all_pixels / len(y_train)))
    ...
    for i, seed in enumerate(range(args.seed, args.seed + args.n_ensemble)):
        ...
        print(f"  {'Epoha':>6}  {'Train ll':>10}  {'LR':>9}")
        print(f"  {'─'*6}  {'─'*10}  {'─'*9}")
        t0 = time.time()
        for epoch in range(1, final_epochs + 1):
            ...
            if epoch % 5 == 0 or epoch in (1, final_epochs):
                print(f"  {epoch:>6}  {total/len(y_all):>10.5f}  "
                      f"{opt_f.param_groups[0]['lr']:>9.2e}")
        print(f"  Treniran v {time.time()-t0:.1f}s")

        # Logiti za crop
        crop_dummy = np.zeros(n_crop, dtype=np.int64)
        crop_ds = DualStreamDataset(data_pca_final, flat_pp_final,
                                     crop_coords, crop_dummy, W, augment=False)
        crop_logits_sum += get_logits_array(model_f, crop_ds, device).astype(np.float64)

    # Ensemble povprečenje logitov + temperature scaling
    crop_logits_avg = (crop_logits_sum / args.n_ensemble).astype(np.float32)
    crop_probs = apply_temperature(crop_logits_avg, T_opt)
    ...
    print(f"  Submission: {args.output}  shape={final_map.shape}  "
          f"T={T_opt:.3f}  σ={args.sigma}")

    # ------------------------------------------------------------------
    print("\n=== POVZETEK ===")
    print(f"  DualStreamCNN v2: N_PCA={N_PCA}, ensemble={args.n_ensemble}")
    print(f"  Best epochs: {best_epochs} (avg={avg_best_epoch})")
    ...
    print(f"\n  Primerjava (TEST):")
    print(f"  {'Dual-Stream v1':25} {'94.41%':>8}  {'0.22209':>10}")
