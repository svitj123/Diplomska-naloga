...
# Submission
# ---------------------------------------------------------------------------
def make_submission(pca, svm, data: np.ndarray, amide_i_idx: int, output_path: str):
    """Ustvari .npy submission datoteko za tekmovalni crop."""
    crop = data[PRED_R0:PRED_R1, PRED_C0:PRED_C1]
    X_pred = crop.reshape(-1, crop.shape[-1])

    print(f"  Preprocessing {len(X_pred)} pikslov za submission...")
    X_pp = preprocess_article_style(X_pred, amide_i_idx)
    X_pca = pca.transform(X_pp)

    probs = svm.predict_proba(X_pca)
    submission = probs.reshape(200, 300, NUM_CLASSES).astype(np.float32)

    np.save(output_path, submission)
    print(f"  Submission shranjen: {output_path}")
    print(f"  Shape: {submission.shape} | dtype: {submission.dtype}")
    print(f"  Vsota verjetnosti (sample pixel): {submission[100, 150].sum():.6f}")
...
def main():
    ...
    make_submission(pca_final, svm_final, data, amide_i_idx, args.output)
