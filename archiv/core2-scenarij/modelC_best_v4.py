...
Model C — Best v4
=================
= v2 features + v3 izboljšave (brez max/min ki so škodili)

Spremembe glede na v2:
  1. Ensemble: 10 modelov (v2: 7) — manjša varianca, boljša kalibracija
  2. Pravi sigma tuning na val prostorni strukturi (v2: fiksno 1.5)
     Rekonstruiramo probability map → Gaussian → log loss na val koordinatah
  3. Min final epochs: 20 (v2: 15)

ENAKO kot v2 (ne menjamo):
  - SPEC_DIM = 561: raw(187) + mean_3(187) + mean_5(187)  ← DOKAZANO DELUJE
  - Patch 17×17, reflect padding
  - StandardScaler + PCA(16)
  - DualStreamCNN arhitektura (~255k param)
  - TTA 8 D4 augmentacij
  - Temperature scaling

Zakaj ne v3 (max/min)?
  v3 VAL ll=0.255 > v2 VAL ll=0.232 — max/min sta noisy statistiki,
  ki pri 30k treningových pikslih overfittata specifični šum slajda.
  T=1.555 (v3) vs T=1.377 (v2) potrjuje slabo kalibracijo v3.
"""
...
SPEC_DIM_RAW = 187
NEIGH_SCALES = [3, 5]
SPEC_DIM     = SPEC_DIM_RAW * (1 + len(NEIGH_SCALES))  # 561
...
# ---------------------------------------------------------------------------
# Neighbourhood mean features (samo mean — dokazano deluje)
# ---------------------------------------------------------------------------
def extract_spec_features(flat_sc, H, W, coords, scales=NEIGH_SCALES):
    """
    [raw(187), mean_3(187), mean_5(187)] = 561 dim za vsak piksel v coords.
    Peak spomin: ~1.8 GB (flat_sc + en mean_map začasno).
    """
    D        = flat_sc.shape[1]
...
def find_best_sigma_spatial(val_logits, y_val, val_coords, T,
    ...
    """
    Pravi sigma tuning (nova v v4):
    Rekonstruiramo full probability map iz val pikslov, apliciramo Gaussian,
    merimo log loss na val koordinatah v njihovem prostornem kontekstu.
    V v2 je bilo sigma=1.5 trdo kodirano brez testiranja.
    """
    ...
    print(f"\n  Sigma tuning na val prostorni strukturi ({len(y_val):,} val pikslov):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Model C Best v4: v2 features + 10 modelov + pravi sigma tuning"
    )
    parser.add_argument("--input",      default="image1-competition.hdf5")
    parser.add_argument("--output",     default="modelC_best_v4.npy")
    ...
    parser.add_argument("--sigma",      type=float, default=-1.0,
                        help="Sigma za Gaussian smoothing. -1 = auto-tune na val (default). "
                             "Nastavi na 1.5 za enako kot v2 (dokazano deluje).")
    ...
    print(f"\n  Konfiguracija:")
    print(f"    SPEC_DIM={SPEC_DIM} (raw {SPEC_DIM_RAW} + mean_3 {SPEC_DIM_RAW} + mean_5 {SPEC_DIM_RAW})")
    print(f"    Patch {PATCH_SIZE}×{PATCH_SIZE} | PCA({N_PCA}) | TTA 8 | Ensemble {args.n_ensemble}")
    ...
    # ------------------------------------------------------------------
    print("\n=== 4. Neighbourhood mean features (561 dim) ===")
    ...
    print("\n=== 7. Sigma ===")
    if args.sigma >= 0:
        # Ročno določena sigma (npr. --sigma 1.5)
        sigma_opt = args.sigma
        print(f"  Sigma fiksna: σ={sigma_opt:.1f} (ročno, override)")
        print(f"  OPOMBA: prostorni sigma tuning je bil metodološko napačen —")
        print(f"    val piksli so razpršeni, luknje zapolnimo s priorom → glajenje izgleda škodljivo.")
        print(f"    Na sklenjenem competition cropu pa glajenje pomaga (dokazano: v2 Final=0.409 z σ=1.5).")
    else:
        # Auto-tune (privzeto — a za production raje --sigma 1.5)
        print("  Auto sigma tuning (opozorilo: metodološko vprašljivo za razpršene val piksle):")
    ...
    print("\n  Refit StandardScaler + PCA na vseh pikslih...")
    flat         = data.reshape(-1, data.shape[-1])
    scaler_final = StandardScaler()
    scaler_final.fit(flat[usable_mask.ravel()])
    pca_final    = PCA(n_components=N_PCA, random_state=args.seed)
    pca_final.fit(scaler_final.transform(flat[usable_mask.ravel()]))
    ...
    print(f"  T={T_opt:.4f} | σ={sigma_opt:.1f} (tuned na val prostorni strukturi)")
    ...
    print(f"    modelC_best_v2: 0.40937  ← naš best")
