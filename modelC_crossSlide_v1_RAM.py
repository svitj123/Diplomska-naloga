"""
Model C — Cross-Slide v1  (pravi train/test TMA split iz clanka)
=====================================================================
...
KLJUCNE RAZLIKE OD v10-v13 (ker so podatki fundamentalno drugacni):
  1. SPEC_DIM_RAW je zdaj 813 (ne 187) — dolocen dinamicno iz podatkov.
  2. Ni vec umetnega KMeans "core" splita — imamo 8 PRAVIH locenih
     prostorskih crop-ov za train. En crop se izbere kot inner-val
     (najmanjsi crop z vsemi 6 razredi — ohrani cim vec train podatkov),
     ostalih 7 je inner-train (Faza A). Vseh 8 je outer-train (Faza B).
  3. Ni vec zunanjega splita (umetni Core2) — test je PRAVA, LOCENA
     datoteka (drug fizicni slajd). En sam dotik na koncu, kot doslej.
  4. Ni StandardScaler (kot v11-v13) — preprocessing (rubber-band+Amide I)
     je ze narejen v sami datoteki (preverjeno: amide_i_normalized=True).
  5. Podatki se procesirajo EN CROP NAENKRAT (ne vsi hkrati v pomnilniku) —
     surovi 813-kanalni canvas je ~2.9GB na crop, 8 hkrati bi zlahka
     porabilo >20GB RAM-a.

Metodologija (Faza A/B nested split) ostaja ista logika kot v10-v13:
...
METODOLOGIJA_OPOMBA = (
    ...
    "sigma sweep na inner-val, mediana za final_epochs (kot v13)."
)


# ---------------------------------------------------------------------------
# Nalaganje — EN CROP NAENKRAT (izogib >20GB RAM pri vseh 8 hkrati)
# ---------------------------------------------------------------------------
...
def load_crop_full(path):
    """Nalozi CEL crop (data+classes+tissue_mask+wns). Klici enega naenkrat!"""
    with h5py.File(path, 'r') as f:
...
# ---------------------------------------------------------------------------
# PCA fit na zbirki anotiranih pikslov iz vec crop-ov (en crop naenkrat)
# ---------------------------------------------------------------------------
def fit_pca_pooled(crop_paths, n_pca=N_PCA, seed=42, label=""):
    ...
    for p in crop_paths:
        ...
        X_ann = flat[ann.ravel()].copy()
        pooled.append(X_ann)
        print(f"    {os.path.basename(p)}: {len(X_ann):,} spektrov ({time.time()-t0:.1f}s)")
        del data, flat
...
# ---------------------------------------------------------------------------
# Procesiranje enega crop-a: PCA transform + neighbourhood spec features
# (samo pri anotiranih koordinatah). Vrne majhne strukture (drzi v pomnilniku).
# ---------------------------------------------------------------------------
def process_crop(path, pca, scales=NEIGH_SCALES, label=""):
    t0 = time.time()
    print(f"  [{label}] Nalagam {os.path.basename(path)} ...")
    with h5py.File(path, 'r') as f:
        data        = np.array(f['data'], dtype=np.float32)
        tissue_mask = np.array(f['tissue_mask'])
        classes     = np.array(f['classes'])
    ...
    print(f"  [{label}] PCA transform ({H*W:,} pikslov)...")
    ...
    print(f"  [{label}] Neighbourhood spec features ({SPEC_DIM} dim, {len(coords):,} tock)...")
    ...
    return {
        ...
        "y": y, "H": H, "W": W, "tissue_mask": tissue_mask, "classes": classes,
    }
...
class MultiCropDataset(Dataset):
    def __init__(self, padded_pca_by_crop, spec_dense, samples, labels,
        ...
        self.samples = samples  # (N,3): crop_id, r, c (r,c v ORIGINALNIH, ne-padded koordinatah)
        self.labels = labels
...
def build_full_canvas_prob_map(H, W, coords, probs, fallback_prior):
    """En crop je ze celo pravokotno platno — ni potreben bbox trik kot pri Core2."""
...
# ---------------------------------------------------------------------------
def main():
    global SPEC_DIM_RAW, SPEC_DIM

    parser = argparse.ArgumentParser(
        description="Model C Cross-Slide v1: pravi train/test TMA split iz clanka"
    )
    ...
    pca_A = fit_pca_pooled(inner_train_paths, N_PCA, args.seed, label="Faza A")

    print(f"\n=== 3. Faza A — procesiranje crop-ov (en naenkrat) ===")
    inner_train_results, inner_train_ids = [], []
    for i, p in enumerate(inner_train_paths):
        res = process_crop(p, pca_A, label=f"inner-train {i}")
        inner_train_results.append(res)
        inner_train_ids.append(i)
    inner_val_res = process_crop(inner_val_path, pca_A, label="inner-val")
    ...
    print("\n=== 5. Faza A — Temperature scaling in evaluacija (na inner-val) ===")
    T_opt = find_temperature(innerval_logits_avg, inner_val_res["y"])
    ...
    print(f"\n=== 8. Faza B — procesiranje vseh train crop-ov ===")
    outer_train_results, outer_train_ids = [], []
    for i, p in enumerate(train_crop_paths):
        res = process_crop(p, pca_B, label=f"outer-train {i}")
    ...
    print(f"\n=== 9. Faza B — procesiranje TEST datoteke (locen fizicni slajd) ===")
    test_res = process_crop(args.test_file, pca_B, label="TEST")
    ...
    print(f"  Faza A: max_epochs={args.max_epochs}, best_epochs={best_epochs}, mediana={final_epochs}")
    print(f"  Faza B: final_epochs={final_epochs}, ensemble={args.n_ensemble}, brez peeka")
