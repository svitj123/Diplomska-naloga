...
def train_blind(padded_pca, samples, y, device, criterion,
    ...
    model     = SingleStreamCNN(n_channels=n_channels, patch_size=patch_size).to(device)
...
def main():
    ...
    n_param = sum(p.numel() for p in SingleStreamCNN(n_channels=n_channels, patch_size=patch_size).parameters()
                 if p.requires_grad)
