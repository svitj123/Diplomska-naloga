...
Model C — Best v3
=================
Spremembe glede na modelC_best_v2.py:

  1. Polne neighbourhood features (kot Vid, samo znotraj CNN frameworka):
     [raw(187)] + [mean_3(187) + std_3(187) + max_3(187) + min_3(187)]
               + [mean_5(187) + std_5(187) + max_5(187) + min_5(187)]
     = 187 × 9 = 1683 dim   ← točno kar je imel Vid v svojem MLP-ju!
     (V v2 smo imeli samo mean_3 + mean_5 = 561 dim)

  2. Sparsa ekstrakcija neighbourhood features:
     Računamo samo za potrebne piksle (train+val+crop ≈ 130k),
     ne za vseh 1.2M H×W pikslov → spomin OK.
     Peak ≈ 2.7 GB med std računanjem (spec**2 + uniform_filter).

  3. Pravi sigma tuning na val prostorni strukturi:
     Rekonstruiramo probability map iz val pikslov, apliciramo Gaussian,
     merimo log loss na val koordinatah. V v2 je bilo σ=1.5 trdo kodirano.

  4. Ensemble: 10 modelov (bilo 7).

  5. Min final epochs: 20 (bilo 15).

Vse ostalo enako kot v2:
  - Patch 17×17, reflect padding
  - StandardScaler + PCA(16)
  - DualStreamCNN (spatial CNN 64 feat + spectral MLP 64 feat)
...
NEIGH_SCALES = [3, 5]
# mean + std + max + min = 4 statistike na skalo
NEIGH_STATS  = 4
# 187 * (1 + 4*2) = 187 * 9 = 1683
SPEC_DIM = SPEC_DIM_RAW * (1 + NEIGH_STATS * len(NEIGH_SCALES))
...
def make_spatial_two_way_split(tissue_mask, classes, prediction_crop_mask,
    ...
    val_cids, train_cids = [], []
    running = 0; toggle = True
    for cid, cnt in comps:
        if running < target_val and toggle:
            val_cids.append(cid); running += cnt
        else:
...
# ---------------------------------------------------------------------------
# Neighbourhood spectral features — SPARSE (samo za potrebne piksle)
# ---------------------------------------------------------------------------
def extract_spec_features(flat_sc, H, W, coords, scales=NEIGH_SCALES):
    """
    Za vsak piksel v coords vrne:
      [raw(D), mean_s1(D), std_s1(D), max_s1(D), min_s1(D),
               mean_s2(D), std_s2(D), max_s2(D), min_s2(D)]
    = D * (1 + 4*len(scales)) dimenzij.

    Spomin: računamo filter na full H×W×D array (908 MB), takoj
    ekstrahiramo samo coords piksle in sprostimo. Peak ≈ 2.7 GB med std.
    """
    D        = flat_sc.shape[1]
    spec_map = flat_sc.reshape(H, W, D)   # view, brez kopiranja
    r, c     = coords[:, 0], coords[:, 1]

    parts = [flat_sc[r * W + c].copy()]   # (N, D) — originalni spekter

    for scale in scales:
        sz = [scale, scale, 1]
        t0 = time.time()
        print(f"    scale={scale}: mean...", end="", flush=True)

        # Mean
        mean_map = uniform_filter(spec_map, size=sz, mode='reflect')
        parts.append(mean_map[r, c].copy())

        # Std = sqrt(E[X²] - E[X]²)
        print(" std...", end="", flush=True)
        sq_mean = uniform_filter(spec_map ** 2, size=sz, mode='reflect')
        std_map = np.sqrt(np.maximum(sq_mean - mean_map ** 2, 0)).astype(np.float32)
        parts.append(std_map[r, c].copy())
        del sq_mean, std_map

        # Max
        print(" max...", end="", flush=True)
        max_map = maximum_filter(spec_map, size=[scale, scale, 1], mode='reflect')
        parts.append(max_map[r, c].copy())
        del max_map, mean_map

        # Min
        print(" min...", end="", flush=True)
        min_map = minimum_filter(spec_map, size=[scale, scale, 1], mode='reflect')
        parts.append(min_map[r, c].copy())
        del min_map

        print(f" {time.time()-t0:.1f}s")

    result = np.concatenate(parts, axis=1).astype(np.float32)
    print(f"    → shape={result.shape}  mem={result.nbytes/1e6:.1f} MB")
    return result  # (N, D*(1+4*len(scales)))
...
class DualStreamDataset(Dataset):
    """
    Vrne (patch, spec_extended, label).
    spec_dense: (N, SPEC_DIM) — pre-ekstrahi neighbourhood features,
                indeksirani po poziciji v datasetu.
    """
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and self.spec_noise > 0:
            # Perturbira samo originalni del spektra (prvih 187 dim)
...
# ---------------------------------------------------------------------------
# Arhitektura: Dual-Stream CNN (SPEC_DIM = 1683)
# ---------------------------------------------------------------------------
class DualStreamCNN(nn.Module):
    """
    Spatial: 17×17×16 PCA patch → CNN → 64 feat
    Spectral: 1683-dim [raw + neigh mean+std+max+min] → MLP → 64 feat
    ...
    def __init__(self, n_channels=N_PCA, spec_dim=SPEC_DIM,
        ...
        # Spectral MLP: 1683 → 256 → 128 → 64
        # (samo prva plast je večja od v2; ostalo enako)
...
def find_best_sigma_spatial(val_logits, y_val, val_coords, T,
                             tissue_mask, H, W,
                             sigmas=(0.0, 0.5, 1.0, 1.5, 2.0, 3.0)):
    """
    Pravi sigma tuning: rekonstruiramo probability map iz val pikslov,
    apliciramo Gaussian, merimo log loss na val koordinatah.
    To da pravo oceno vpliva sigma — v v2 smo sigma fiksirali na 1.5.
    """
    val_probs = apply_temperature(val_logits, T)

    # Prior za ne-val piksle v mapi
    prior = np.bincount(y_val, minlength=NUM_CLASSES).astype(np.float32)
    prior /= prior.sum()

    r, c = val_coords[:, 0], val_coords[:, 1]
    best_sigma, best_ll = 0.0, float('inf')

    print(f"\n  Sigma tuning na val prostorni strukturi ({len(y_val):,} pikslov):")
    for sigma in sigmas:
        if sigma <= 0:
            ll = log_loss(y_val, val_probs, labels=np.arange(NUM_CLASSES))
        else:
            # Rekonstruiraj full probability map
            prob_map = np.zeros((H, W, NUM_CLASSES), dtype=np.float32)
            prob_map[:] = prior           # inicializiraj s priorom
            prob_map[r, c] = val_probs    # vstavi val verjetnosti

            # Gaussian smoothing
            smoothed = gaussian_smooth_probs(prob_map, tissue_mask, sigma)

            # Log loss samo na val pikslih
            val_probs_sm = np.clip(smoothed[r, c], 1e-9, 1.0)
            val_probs_sm /= val_probs_sm.sum(axis=1, keepdims=True)
            ll = log_loss(y_val, val_probs_sm, labels=np.arange(NUM_CLASSES))

        marker = " ←" if ll < best_ll else ""
        print(f"    σ={sigma:.1f}: val ll={ll:.5f}{marker}")
        if ll < best_ll:
            best_ll, best_sigma = ll, sigma

    print(f"  Optimalni σ={best_sigma:.1f} (val ll={best_ll:.5f})")
    return best_sigma
...
def main():
    parser = argparse.ArgumentParser(
        description="Model C Best v3: dual-stream + full neighbourhood (mean+std+max+min)"
    )
    parser.add_argument("--input",      default="image1-competition.hdf5")
    parser.add_argument("--output",     default="modelC_best_v3.npy")
    ...
    print(f"    Spektralni tok: {SPEC_DIM_RAW} raw + "
          f"{SPEC_DIM-SPEC_DIM_RAW} neighbourhood (mean+std+max+min @ {NEIGH_SCALES})"
          f" = {SPEC_DIM} dim")
    ...
    # ------------------------------------------------------------------
    print("\n=== 4. Neighbourhood features — sparse ekstrakcija ===")
    print(f"  SPEC_DIM = {SPEC_DIM} (raw {SPEC_DIM_RAW} + neigh {SPEC_DIM-SPEC_DIM_RAW})")
    ...
    # ------------------------------------------------------------------
    print("\n=== 7. Sigma tuning (pravi — prostorna struktura) ===")
    sigma_opt = find_best_sigma_spatial(
        val_logits_avg, y_val, val_coords, T_opt,
        tissue_mask, H, W,
        sigmas=(0.0, 0.5, 1.0, 1.5, 2.0, 3.0)
    )
    ...
    print(f"  Ref v2:  VAL ll=0.23217 | Final=0.409")

    # ------------------------------------------------------------------
    n_all = int(((classes != -1) & (~prediction_crop_mask)).sum())
    proportional = round(avg_best_epoch * n_all / max(len(y_train), 1))
    final_epochs = max(proportional, 20)   # min 20 epoh
    print(f"\n=== 9. Finalni ensemble (100%, {final_epochs} epoh × {args.n_ensemble} modelov) ===")
    print(f"  (proporcionalno={proportional}, min=20 → final_epochs={final_epochs})")
    ...
    # Neighbourhood features za final (sparse)
    ...
    print(f"\n  Neighbourhood features za all_coords ({len(all_coords):,} pikslov):")
    all_spec_final  = extract_spec_features(flat_sc_final, H, W, all_coords,  NEIGH_SCALES)
    print(f"  Neighbourhood features za crop ({n_crop:,} pikslov):")
    ...
    for i, seed in enumerate(seeds):
        ...
        final_ldr = DataLoader(final_ds, batch_size=args.batch_size, shuffle=True, num_workers=0)
    ...
    print(f"  Gaussian smoothing (sigma={sigma_opt:.1f})...")
    if sigma_opt > 0:
    ...
    print(f"  T={T_opt:.4f} | σ={sigma_opt:.1f} (tuned)")
    print(f"  VAL OA={val_oa*100:.2f}%  VAL ll={val_ll:.5f}")
    print(f"\n  Primerjava (Final — nižje je boljše):")
    print(f"    Zmagovalec:    0.40031")
    print(f"    2. mesto:      0.40813")
    print(f"    modelC_best_v2: 0.40937  ← naš prejšnji best")
