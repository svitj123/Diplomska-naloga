...
class SingleStreamCNN(nn.Module):
    def __init__(self, n_channels=N_PCA, num_classes=NUM_CLASSES, dropout=0.5):
        ...
        # Patch 17x17 -> po dveh MaxPool(2) je prostorska dimenzija 4x4
        # (17 -> pad ohranja 17 -> pool 8 -> pad ohranja 8 -> pool 4).
...
def train_blind(padded_pca, samples, y, device, criterion,
    ...
    model     = SingleStreamCNN(n_channels=n_channels).to(device)
...
def main():
    ...
    n_param = sum(p.numel() for p in SingleStreamCNN(n_channels=n_channels).parameters()
                 if p.requires_grad)
    ...
    for r_idx, inner_val_path in enumerate(rotation_candidates):
        ...
        iv_ds = PatchDataset(padded_iv, samples_iv, iv_res["y"], augment=False)
        del it_results
        ...
        for i, seed in enumerate(calib_seeds):
            ...
            model_c = train_blind(
                ...
                lr=args.lr, seed=seed, n_channels=n_channels,
            )
    ...
    padded_test = {cid: pad_pca(res["data_pca"]) for cid, res in zip(test_ids, test_results)}
